...
export class CarouselDirective<T> implements OnInit, OnDestroy {
	@Input() set appCarousel(items: T[] | undefined) {
		...
		setInterval(() => {
			this, this.currentIndex$.next(this.currentIndex$.value + 1);
		}, 2000);
	...
	constructor(
		...
		private readonly template: TemplateRef<ICorouselContext<T>>,
	) {}
	...
	private listenCurrentIndexChange() {
		this.currentIndex$
			.pipe(
				map((currentIndex: number) => this.getCurrentContext(currentIndex)),
	...
	private getCurrentContext(currentIndex: number): ICorouselContext<T> {
		return {
